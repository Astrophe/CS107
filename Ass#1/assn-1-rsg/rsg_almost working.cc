/**
 * This implementation is closer to ideal, but I could not get line 54 to work: 
 * Production currRandomProduction = grammar[currentNonTerminal].getRandomProduction();
 * grammar[currentNonTerminal] kept returning an empty definition which caused the
 * getRandomProduction method to fail; even though the currentNonTerminal value was correct,
 * and I was simply using it as the key to access the prepopulated grammar?
...
/**
 * Recursive function that generates the sentences
 */

static void randomSentenceGeneratorRec(vector<string>& randomSentence, string currentNonTerminal, map<string, Definition>& grammar)
{
   Production currRandomProduction = grammar[currentNonTerminal].getRandomProduction();
    
   for (Production::iterator curr = currRandomProduction.begin(); curr != currRandomProduction.end(); ++curr) {
     if (grammar.count(*curr) > 0) {
       randomSentenceGeneratorRec(randomSentence, *curr, grammar);
     }
     else {
       randomSentence.push_back(*curr);
     }
   }
}
...
int main(int argc, char *argv[])
{
  ...
  string currentNonTerminal = "<string>";
  randomSentenceGeneratorRec(randomSentence, currentNonTerminal, grammar);
  cout << randomSentence.size() << endl;
  
  // Print sentence
  for (std::vector<string>::const_iterator i = randomSentence.begin(); i != randomSentence.end(); i++) {
    std::cout << *i << " " << endl;
  }
