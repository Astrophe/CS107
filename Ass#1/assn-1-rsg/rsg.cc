...
static void randomSentenceGeneratorRec(ifstream& infile, vector<string>& randomSentence, string currentNonTerminal, map<string, Definition>& grammar)
{
   string uselessText;
   getline(infile, uselessText, '{');
   //infile.putback(currentNonTerminal);
   infile.putback('{');
   Definition currentDef(infile);
   //Definition currentDef(grammar[currentNonTerminal]);
   Production currRandomProduction = currentDef.getRandomProduction();
   // Production currRandomProduction;
    
   for (Production::iterator curr = currRandomProduction.begin(); curr != currRandomProduction.end(); ++curr) {
     if (grammar.count(*curr) > 0) {
       randomSentenceGeneratorRec(infile, randomSentence, *curr, grammar);
     }
     else {
...
int main(int argc, char *argv[])
{
  ...
  ifstream secondGrammarFile(argv[1]);
  randomSentenceGeneratorRec(secondGrammarFile, randomSentence, currentNonTerminal, grammar);
